...
/// Service for managing profile images including selection, compression, and storage
class ImageService {
  ...
  Future<String> processAndSaveImage(File imageFile) async {
    try {
      ...
      if (!await imageFile.exists()) {
        throw ImageServiceException('Image file does not exist');
      }

      // Check if it's a valid image file
      final bytes = await imageFile.readAsBytes();
      if (!await _isValidImage(bytes)) {
        throw ImageServiceException('Selected file is not a valid image');
      }
      ...
      throw ImageServiceException('Failed to process image: ${e.toString()}');
    }
  }
